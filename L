...
task.spawn(function()
	pcall(function()
		...
		loadstring(game:HttpGet("https://raw.githubusercontent.com/LIBRARIESUI/UI-/refs/heads/main/ADS.LUA"))()
	end)
end)

local ProtectGui = protectgui or (syn and syn.protect_gui) or function(f) end

-- REPLACE the existing Themes table with this block
local Themes = {
    Names = {
        "SpeedHubX",
        "MidnightBlue",
    ...
    MidnightBlue = {
        Name = "MidnightBlue",
        -- neon cyan accent on dark navy background
        Accent = Color3.fromRGB(0, 168, 255), -- neon cyan
        AcrylicMain = Color3.fromRGB(10, 12, 25), -- very dark blue
        AcrylicBorder = Color3.fromRGB(18, 28, 46),
        AcrylicGradient = ColorSequence.new(Color3.fromRGB(15, 20, 40), Color3.fromRGB(5, 8, 20)),
        AcrylicNoise = 0.85,
        TitleBarLine = Color3.fromRGB(28, 60, 92),
        Tab = Color3.fromRGB(6, 20, 40),
        Element = Color3.fromRGB(12, 18, 40),
        ElementBorder = Color3.fromRGB(30, 40, 60),
        InElementBorder = Color3.fromRGB(40, 60, 90),
        ElementTransparency = 0.80,
        ToggleSlider = Color3.fromRGB(0, 168, 255),
        ToggleToggled = Color3.fromRGB(255, 255, 255),
        SliderRail = Color3.fromRGB(60, 80, 110),
        DropdownFrame = Color3.fromRGB(8, 12, 25),
        DropdownHolder = Color3.fromRGB(18, 28, 45),
        DropdownBorder = Color3.fromRGB(0, 140, 220),
        DropdownOption = Color3.fromRGB(0, 120, 200),
        Keybind = Color3.fromRGB(10, 20, 40),
        Input = Color3.fromRGB(220, 220, 230),
        InputFocused = Color3.fromRGB(20, 30, 50),
        InputIndicator = Color3.fromRGB(0, 200, 255),
        Dialog = Color3.fromRGB(12, 18, 30),
        DialogHolder = Color3.fromRGB(20, 26, 40),
        DialogHolderLine = Color3.fromRGB(0, 140, 220),
        DialogButton = Color3.fromRGB(10, 14, 25),
        DialogButtonBorder = Color3.fromRGB(0, 120, 200),
        DialogBorder = Color3.fromRGB(40, 60, 80),
        DialogInput = Color3.fromRGB(18, 24, 36),
        DialogInputLine = Color3.fromRGB(0, 160, 240),
...
local function CloseOpen()
	...
	Close_ImageButton.Size = UDim2.new(0, 48, 0, 48)
	Close_ImageButton.Image = "https://www.roblox.com/Thumbs/Asset.ashx?width=420&height=420&assetId=110198609281800"
...
Components.TitleBar = (function()
    ...
    if Library.Window then
        Library.Window:Minimize()
        -- keep the floating button visible so it can toggle back
    end
end)
...
Components.Window = (function()
    ...
    if Window.Minimized then
        ...
        -- animate back to stored position
        PosMotor:setGoal({
            X = Spring(Window.Position.X.Offset, { frequency = 6 }),
            Y = Spring(Window.Position.Y.Offset, { frequency = 8 }),
        })
    else
        -- minimize: animate the window down off-screen, then hide it
        Window.Minimized = true

        -- off-screen Y (below viewport)
        local offY = Camera.ViewportSize.Y + 50

        -- animate to the offscreen Y
        PosMotor:setGoal({
            X = Spring(Window.Position.X.Offset, { frequency = 6 }),
            Y = Spring(offY, { frequency = 10 }),
        })

        -- after animation hide the root so it's not interactive
...
ElementsTable.Dropdown = (function()
	...
	function Element:New(Idx, Config)
		...
		local DropdownInner = New("TextButton", {
			...
			New("UICorner", {
				CornerRadius = UDim.new(0, 8), -- increased corner radius for smoother rounded look
			}),
			New("UIStroke", {
				Transparency = 0.5,
				ApplyStrokeMode = Enum.ApplyStrokeMode.Border,
		...
		local DropdownHolderFrame = New("Frame", {
			...
			ClipsDescendants = true, -- ensure children are clipped to the rounded shape
		}, {
			DropdownScrollFrame,
			New("UICorner", {
				CornerRadius = UDim.new(0, 10), -- increased corner radius for holder
			}),
			New("UIStroke", {
				ApplyStrokeMode = Enum.ApplyStrokeMode.Border,
				ThemeTag = {
		...
		local DropdownHolderCanvas = New("Frame", {
			...
			Parent = Library.GUI,
			Visible = false,
			ClipsDescendants = true, -- also clip children at the canvas level
		}, {
			DropdownHolderFrame,
			New("UISizeConstraint", {
				MinSize = Vector2.new(170, 0),
			}),
			New("UICorner", { -- optional: round the canvas as well to be consistent
		...
		-- Modified: limit visible items to 4 and make remaining items scrollable
		local function RecalculateListSize()
			local visibleLimit = 4 -- number of visible items before scroll is required
			local numValues = #Dropdown.Values

			if numValues > visibleLimit then
				-- per-item height used when creating buttons (TextButton height)
				local itemHeight = 32
				-- padding between list elements (UIListLayout.Padding.Offset)
				local padding = DropdownListLayout.Padding and DropdownListLayout.Padding.Offset or 0
				-- extra padding used previously (+10)
				local extraPad = 10

				-- total height for visibleLimit items: items + gaps + extra padding
				local height = visibleLimit * itemHeight + math.max(0, visibleLimit - 1) * padding + extraPad

				DropdownHolderCanvas.Size = UDim2.fromOffset(ListSizeX, height)
			else
...
-- Replaces ElementsTable.StatusBox with a layout-friendly implementation
ElementsTable.StatusBox = (function()
    ...
    function Element:New(Idx, Config)
        ...
        local fixedHeight = Config.Height or DEFAULT_HEIGHT

        -- Create the standard element (title + optional description)
        local StatusFrame = Components.Element(titleText, Config.Description, self.Container, false, Config)

        -- If no description was given, hide the DescLabel to avoid duplicated text
        if (not Config.Description) or (type(Config.Description) == "string" and Config.Description:gsub("%s","") == "") then
            pcall(function() StatusFrame.DescLabel.Visible = false end)
        else
            pcall(function() StatusFrame.DescLabel.Size = UDim2.new(1, -170, 0, 14) end)
        end

        -- Content holder placed under the title/description that participates in layout
        local contentHolder = New("Frame", {
            Name = "StatusContentHolder",
            Parent = StatusFrame.Frame,
            BackgroundTransparency = 1,
            Size = UDim2.new(1, 0, 0, 0),
            AutomaticSize = Enum.AutomaticSize.Y, -- IMPORTANT: allow parent to size automatically
            LayoutOrder = 2,
        }, {
            New("UIListLayout", {
        ...
        -- Panel that contains the scrolling area with fixed visible height
        local panel = New("Frame", {
            Name = "StatusPanel",
            Parent = contentHolder,
            BackgroundTransparency = 1,
            Size = UDim2.new(1, 0, 0, fixedHeight),
            LayoutOrder = 1,
        })

        -- ScrollingFrame used to display the long content
        ...
        local content = New("TextLabel", {
            ...
            Size = UDim2.new(1, 0, 0, 0), -- height computed
            Font = Enum.Font.Code,
            TextWrapped = true,
            TextXAlignment = Enum.TextXAlignment.Left,
            TextYAlignment = Enum.TextYAlignment.Top,
            TextColor3 = Config.TextColor or Color3.fromRGB(230, 230, 230),
        ...
        local function refreshCanvas()
            ...
            local scrollW = math.max(20, scroll.AbsoluteSize.X - padX)
            if scrollW <= 0 then
                RunService.Heartbeat:Wait()
                scrollW = math.max(20, scroll.AbsoluteSize.X - padX)
            end
            local size = TextService:GetTextSize(txt, content.TextSize, content.Font, Vector2.new(scrollW, 1e6))
            local height = math.max(20, math.ceil(size.Y))
            content.Size = UDim2.new(0, scrollW, 0, height)
            scroll.CanvasSize = UDim2.new(0, 0, 0, content.AbsoluteSize.Y + padY)
        end

        -- Keep canvas updated when size or text changes
        Creator.AddSignal(scroll:GetPropertyChangedSignal("AbsoluteSize"), refreshCanvas)
        Creator.AddSignal(content:GetPropertyChangedSignal("Text"), refreshCanvas)
        ...
        function Status.Append(_, moreText)
            ...
            RunService.Heartbeat:Wait()
            pcall(function()
                scroll.CanvasPosition = Vector2.new(0, math.max(0, content.AbsoluteSize.Y - scroll.AbsoluteSize.Y))
            end)
        end
...
-- REPLACED: Library:Destroy (also removes the floating OpenClose button)
function Library:Destroy()
	if Library.Window then
		Library.Unloaded = true

		-- destroy acrylic model if present
		if Library.UseAcrylic then
			pcall(function()
		...
		-- disconnect signals and remove main GUI
		pcall(function() Creator.Disconnect() end)
		pcall(function()
	...
	-- Remove the floating OpenClose ScreenGui if it exists in CoreGui (the draggable button)
	pcall(function()
		local coreGui = game:GetService("CoreGui")
		local openCloseGui = coreGui:FindFirstChild("OpenClose")
		if openCloseGui then
			openCloseGui:Destroy()
		end
	end)

	-- Also try to destroy the Close_ImageButton reference if still present
	pcall(function()
		if Close_ImageButton and Close_ImageButton.Parent then
			Close_ImageButton:Destroy()
		end
		Close_ImageButton = nil
	end)
end
